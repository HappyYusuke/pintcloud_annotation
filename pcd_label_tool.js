...
function deleteObject(bboxClass, trackId, labelIndex) {
    ...
    labelTool.cubeArray[labelTool.currentFileIndex].splice(labelIndex, 1); // ★★★ この行を追加 ★★★
...
function addBoundingBoxGui(bbox, bboxEndParams) {
    ...
    let cubeWidth = folderSize.add(bbox, 'width').name("width").min(0.3).max(3).step(0.01).listen();
    let cubeLength = folderSize.add(bbox, 'length').name("length").min(0.3).max(3).step(0.01).listen();
    let cubeHeight = folderSize.add(bbox, 'height').name("height").min(0.3).max(3).step(0.01).listen();
    folderSize.close();
    ...
    if (bboxEndParams !== undefined && interpolationMode === true) {
        ...
        if (interpolationStartFileIndex !== bboxEndParams.newFileIndex) {
            ...
            let cubeEndWidth = labelTool.folderEndSize.add(bboxEndParams, 'width').name("width").min(0.3).max(3).step(0.01).listen();
            let cubeEndLength = labelTool.folderEndSize.add(bboxEndParams, 'length').name("length").min(0.3).max(3).step(0.01).listen();
            let cubeEndHeight = labelTool.folderEndSize.add(bboxEndParams, 'height').name("height").min(0.3).max(3).step(0.01).listen();
...
function keyDownHandler(event) {
    if (labelTool.selectedMesh !== undefined) {
        switch (event.keyCode) {
            ...
            case 46: // Deleteキー
            case 8:  // Backspaceキー
                // 選択されているオブジェクトのインデックスを取得
                let labelIndex = getObjectIndexByName(labelTool.selectedMesh.name);

                if (labelIndex !== -1) {
                    // アノテーション情報からクラス名とtrackIdを取得
                    let annotationObj = annotationObjects.contents[labelTool.currentFileIndex][labelIndex];
                    let bboxClass = annotationObj["class"];
                    let trackId = annotationObj["trackId"];
...
function setOrthographicView() {
    ...
    currentCamera = new THREE.OrthographicCamera(-10, 10, 5, -5, 0.0001, 2000);
...
function mouseDownLogic(ev) {
    ...
    if (clickedObjects.length > 0) {
        ...
        if (ev.button === 0) {
            ...
            // ==========================================================
            // 右クリックでバウンディングボックスを削除しないようにコメントアウト
            // ==========================================================
            /*
            } else if (ev.button === 2) {
                // rightclick
